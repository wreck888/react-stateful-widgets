...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    for ( let i = 0; i < progList.length; i++){
      if ( progList[i].id === progName){
        return progList[i].name
      }
    }
  };
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: progName ? 'gold' : 'royalblue', // 🤔 color turns to gold, when celebrating
  };
      ...
      <div className='programmers'>
        {
          ...
          progList.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() =>   setprogName(dev.id) }>Feature</button>
            </div>
          )
        }
      </div>
